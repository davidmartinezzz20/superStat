...
Deno.serve(async (req) => {
  ...
  if (!Deno.env.get('STRIPE_SECRET_KEY')) {
    ...
    return json(req, { error: 'pago-sin-configurar' }, 500);
  }
  ...
  try {
    if (accion === 'gestionar') {
      ...
      if (!clienteExistente) return json(req, { error: 'sin-suscripcion' }, 400);
    ...
    if (accion !== 'suscribir') return json(req, { error: 'accion-desconocida' }, 400);
    ...
    if (!sesion.url) return json(req, { error: 'sin-url-de-pago' }, 500);
    return json(req, { url: sesion.url });
  } catch (e) {
    console.error('Stripe falló al abrir ' + accion, e);
    return json(req, { error: 'stripe-no-responde' }, 502);
  }
});
